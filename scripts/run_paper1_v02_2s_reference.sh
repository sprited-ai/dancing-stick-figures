#!/usr/bin/env bash
# Official v0.2 2-second (40-frame, 20 fps, stride-1, seed-disjoint) reference pipeline.
# Stage 1: text-conditioned image DiT, 30k steps.
# Stage 2: factorised video DiT, image-warm-started, 40 frames, 10k steps.
# Stage 3: matched random-init video DiT, identical protocol minus --init.
set -euo pipefail

TASK_ROOT="${TASK_ROOT:-/data/dancing-stick-figure-paper}"
CACHE="${CACHE:-$TASK_ROOT/cache/mini_v02_2s}"
IMAGE_RUN="${IMAGE_RUN:-$TASK_ROOT/results/paper1_v02_2s_t2i30k_s0}"
WARM_RUN="${WARM_RUN:-$TASK_ROOT/results/paper1_v02_2s_t2v40_warm10k_s0}"
RAND_RUN="${RAND_RUN:-$TASK_ROOT/results/paper1_v02_2s_t2v40_rand10k_s0}"
...
python3 -u -m train.video_dit_fm \
  ...
  --size 64 --frames 1 --stride 1 --patch 4 --dim 384 --depth 12 --heads 6 \
...
video_stage() {
  ...
  python3 -u -m train.video_dit_fm \
    ...
    --size 64 --frames 40 --stride 1 --patch 4 --dim 384 --depth 12 --heads 6 \
    --cond text --cfg_drop 0.1 --text_encoder google-t5/t5-small --text_len 32 \
...
touch "$TASK_ROOT/results/PAPER1_V02_2S_REFERENCE_DONE"
