...
\begin{abstract}
Learning video generation by training models yourself is hard. Real video datasets are large, the process
behind each frame is unknown, and one aggregate score rarely says why a generated video failed. \textbf{Dancing
Stick Figures} is a synthetic video dataset with a deterministic renderer, built so that the whole experiment
fits on one GPU. It contains 1,340 six-second motions rendered from three cameras. Every frame stays linked to
