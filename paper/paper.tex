...
Stated as a research question: \emph{can a deliberately narrow, fully controlled synthetic domain support a
benchmark in which visible structural failures of video generation are measured directly, canonical design
choices are resolved under matched single-GPU budgets, and an observed failure is attributed to a specific
component of the generation stack?} The dataset, evaluation suite, and reference ladder below are our answer,
and the controlled-corruption study reports where the answer's instruments themselves fail.
