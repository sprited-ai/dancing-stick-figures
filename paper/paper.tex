...
\begin{abstract}
Training a video generation model is hard to learn by doing. Real video datasets are large, the process behind
each frame is unknown, and one aggregate score rarely says why a generated video failed. \textbf{Dancing Stick
Figures} is a synthetic video dataset with a deterministic renderer, built so that the whole experiment fits on
one GPU. It contains 1,340 six-second motions rendered from three cameras: 4,020 videos and 482,400 labelled
frames. Every frame stays linked to the joints, camera, body parameters, depth, surface normals, part labels,
and source motion that produced it, so evaluation has an answer key. Rule-based measurements check whether
coloured limbs stay connected and how position and pose change over time. Controlled corruptions test what each
measurement can and cannot see; in a 120-frame study, playing videos backward leaves the time-symmetric motion
signals unchanged and scores FVD 79.3 where real--real scores 80.5. The release includes a 0.85-GB $64^2$
training tier, a $32^2$ route, fixed evaluation manifests, checkpoints, and a Colab lesson that trains an image
generator and then a video generator.
\end{abstract}

\section{Introduction}
Video generation is easy to encounter and hard to study. Systems such as Seedance and MiniMax
H3~\cite{gao2025seedance,minimax2026h3} publish results and sometimes weights, but a learner still runs into
three walls:
\begin{itemize}[leftmargin=*,nosep]
\item \textbf{Data.} Without the training data, a reader cannot reconstruct the learning problem or trace a
failure back to it. The exact data, filters, and recipe behind a strong model are rarely released together.
\item \textbf{Experimentation.} Frontier training needs large corpora, heavy compute, and pretrained codecs and
encoders. What remains affordable is prompting and fine-tuning. Retraining a controlled variant in an afternoon
is not.
\item \textbf{Evaluation.} ``Better'' can mean cleaner frames, an intact body, plausible motion, or prompt
adherence, and these change independently. An aggregate score does not say which one changed or which component
broke it.
\end{itemize}
...
experiments? That requires three things. The full image-to-video path must fit one consumer GPU or a Colab
runtime. A run must produce inspectable multi-second samples in hours, not days. And evaluation must measure
visible failures directly, with known blind spots.

Dancing Stick Figures is built to those constraints. It is a \emph{dataset and renderer}: articulated motions
whose rendered frames stay connected to their exact generating state. It is an \emph{evaluation suite}: fixed
manifests, rule-based structural measurements, and controlled corruptions that test the measurements
themselves. And it ships reference models and a Colab workflow that trains an image generator, transfers its
spatial weights to a video generator, trains it, samples multi-second videos, and diagnoses the failures.

The release contributes:
\begin{enumerate}
\item a label-rich video dataset with a 0.85-GB $64^2$ tier, an optional $32^2$ tier, the motion, camera, and
...
Several lines of work provide synthetic video, human-motion supervision, or decomposed evaluation. Dancing
Stick Figures combines data, renderer, reference models, and measurements in one experiment a reader can rerun
and alter.
...
not aim to make pretraining itself a short experiment. Our 0.85-GB tier makes the opposite trade: narrow visual
...
support motion modelling but specify no pixel renderer or visual evaluation. We use articulated motion as
source material and simplify appearance so that generated pixels can be related to joints and limbs.
...
this domain: near-static failures (\emph{sways to slow music}), motion the renderer cannot show (\emph{shakes
their head no}; the head is a filled circle), absent or unrecognisable actions (\emph{yoga warrior pose},
\emph{yoga tree pose}, \emph{sit up}, \emph{push up}, \emph{standing long jump}), and interactions with objects
that do not exist in the render (\emph{climbs over a low wall}, \emph{lies down on the floor}). The release
...
The configurations are compute tiers of one dataset. $128^2$ keeps the most detail; the 0.85-GB $64^2$ tier is
the primary training tier; $32^2$ supports smoke tests and rapid ablations. Reported structural results use
$64^2$, where the thin coloured limbs remain measurable.

The motion release supports rebuilding the full visual dataset without ARDY: a script re-derives body and
camera settings from each clip identifier and reruns the public renderer. A verifier checks metadata exactly
and compares rendered colour, segmentation, visibility, depth, and normals within declared tolerances.
...
clip. The reference models train on a declared window view: 40 consecutive frames at a random offset within the
first 64 frames of each clip. The 40-frame length matches ARDY's two-second generation window and keeps the
training tensor small; the first-64-frame restriction concentrates training where the prompted action occurs,
because ARDY does not pace an action to the requested duration. The window rows in
Table~\ref{tab:backbone-results} compare generated windows with real windows drawn under the same manifest
rule.
...
% REMAINING(2026-08-26): local-mixer and joint-attention pixel rows land tonight (day queue).
\end{table*}
...
TVR alone and by FVD alone gives opposite orderings.
% REMAINING: extend with the local-mixer and joint-attention pixel rows when their evals land.
...
approximately. The earlier UNet lesson completed on a hosted Tesla T4; hardware numbers are reported for the
...
biomechanics. The motions inherit ARDY's capabilities and biases; the nine-prompt curation removed what
inspection found, but we have not human-validated every remaining motion, and ARDY does not pace actions to the
requested duration. The seed-disjoint test measures in-domain generation under unseen motion realisations, not
...
The most direct extension is a learned rig estimator with calibrated per-joint confidence: map a generated
video back to a rig, re-render it, and compare. That would expose rig-space failures (bone length, joint
angles, foot contact) and rendered-space disagreement, but its scores would need validation on controlled
malformations before use as evidence.
