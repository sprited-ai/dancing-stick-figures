...
\caption{One released motion viewed through the dataset. (a) The same pose from three deterministic cameras. (b) Five
frames spanning its 120-frame clip. (c) The RGBA frame is aligned with part segmentation, depth, camera-space normals,
...
rectified-flow velocity prediction~\cite{peebles2023dit,lipman2023flow}, and a frozen T5-small encoder supplying
...
video model from random initialisation (what joint temporal training must learn by itself), then trains an
identically configured video model whose compatible spatial and text weights are copied from the image checkpoint
(what spatial transfer contributes; temporal positions and temporal attention still learn from video). Two design
contrasts complete the ladder under the same trainer, data view, and step budget. The first varies the attention
factorisation: a DiT whose every block attends jointly over all spatio-temporal tokens instead of alternating the two
axes---the direct-interaction design used by larger systems, at roughly four times the attention compute here. The
second varies the representation: a matched DiT trained in the latent space of a separately trained video
autoencoder, whose generated latents are decoded before evaluation so that codec reconstruction cost is part of its
scores. Table~\ref{tab:backbone-results} collects these runs; each block of
...
first-64-frame view with the same rectified-flow trainer, optimiser, and step budget; one variable changes per rung.
...
\multicolumn{10}{@{}l}{\textit{Single frames, prompt-conditioned, $n=512$, 30 sampling steps}}\\
Real validation frames & --- & --- & --- & --- & --- & --- & --- & --- & --- \\
Image DiT, 30k & factorised & random & --- & --- & --- & --- & --- & --- & --- \\
\addlinespace
...
Video DiT, 10k & factorised & random & --- & --- & --- & --- & --- & --- & --- \\
Video DiT, 10k & factorised & image & --- & --- & --- & --- & --- & --- & --- \\
Video DiT, 10k & joint spatio-temporal & image & --- & --- & --- & --- & --- & --- & --- \\
Latent video DiT, 10k & factorised & random & --- & --- & --- & --- & --- & --- & --- \\\bottomrule
\end{tabular}
...
real--real value of the two disjoint reference halves.
% TODO(fill tonight): single-frame real/model rows from eval.score_images; video model rows from the win64 protocol
% evaluations of paper1_v02c_{t2v64_rand10k,t2v64_warm10k,resunet...,fullst...}; refresh real row from the re-run
% degenerate_64f_n128.json (train_replay first_frames fix does not change real halves).
\end{table*}

% TODO(fill tonight): replace with the measured ladder reading (random vs image init; ResUNet and joint-attention
% contrasts) once the win64 evaluations of the four video runs complete.
