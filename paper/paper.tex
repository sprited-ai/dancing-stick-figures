...
Stated as a research question: \emph{can people train small video generation models from scratch on a cheap but
meaningful dataset, and iteratively study the behaviour of those models?} For the study to be iterative rather
than anecdotal, the domain must make three things possible under a single-GPU budget: visible structural
failures must be measured directly, canonical design choices must resolve under matched budgets, and an
observed failure must be attributable to a specific component of the generation stack. The dataset, evaluation
suite, and reference ladder below are our answer, and the controlled-corruption study reports where the
answer's instruments themselves fail.
...
\paragraph{Reading the ladder.} The initialisation rung resolves decisively at this budget: from random
initialisation the 10k-step video model violates topology in $94\%$ of frames (TVR .942, FVD 1641), while the
same model warm-started from the image checkpoint reaches TVR .169 against the real rows' .116, an order of
magnitude fewer violations for identical video-stage compute. A seed repeat of the warm-started run lands at
TVR .183 and FVD 570.7 versus 545.0, so the reported contrasts are large against training-seed variance. The
table's second lesson is that the two measurement families disagree informatively rather than redundantly: the
warm-started pixel model has near-real structure but overshoots motion (centroid speed .850 versus the real
.373), which FVD punishes heavily; the latent rows show the reverse signature---FVD in the low 500s but TVR
above .8, structurally broken figures rendered smoothly through the codec. A reader ranking these models by
either number alone would reach opposite conclusions, which is precisely the failure-disentangling behaviour
