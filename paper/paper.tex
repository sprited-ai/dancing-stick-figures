...
contrasts complete the ladder under the same trainer, data view, and step budget. The first varies the mechanism
...
attention over all spatio-temporal tokens---the direct-interaction design used by larger systems. At pixel scale
(16{,}384 tokens) joint attention costs $4.7\times$ the factorised step in measured wall-clock, so we report that
contrast in latent space, where 1{,}024 tokens make it essentially free ($0.17$ vs.\ $0.19$\,s/step measured); the
step-matched pixel-scale run is deferred to a revision. The second axis varies the representation: matched DiTs
trained in the
...
estimate; the floor row bounds how much this can matter. Table~\ref{tab:backbone-results} collects these runs; each block of
...
Latent video DiT, 10k & factorised & random & --- & --- & --- & --- & --- & --- & --- \\
Latent video DiT, 10k & joint spatio-temporal & random & --- & --- & --- & --- & --- & --- & --- \\\bottomrule
\end{tabular}
...
% evaluations of paper1_v02c_{t2v64_rand10k,t2v64_warm10k,t2v64_local3d_warm10k,
% t2v16lat_rand10k,t2v16lat_fullst_rand10k} (pixel full-ST deferred to revision); refresh real row from the re-run degenerate_64f_n128.json
% (train_replay first_frames fix does not change real halves). Also report the L3 seed-1 repeat as a
% training-seed variance note under the table.
\end{table*}
