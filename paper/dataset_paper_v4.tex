...
can change one variable at a time, rerender the clip, and observe which diagnostics react.
...
The released motion table lets users rebuild or vary the rendered
...
\paragraph{Released reference models.} Every released denoiser uses velocity-prediction flow matching on a
linear interpolant (often called rectified flow, though we apply no reflow step)~\cite{peebles2023dit,lipman2023flow},
but the losses are not identical. The image and pixel-video DiTs minimise velocity MSE with each pixel
weighted by $1+\alpha$, where $\alpha$ is the ground-truth soft alpha, and normalise by the mean weight.
Mini-Wan uses the latent-space and decoded-space losses described below. Each model uses a frozen T5-small
encoder attached through cross-attention.
\begin{itemize}[leftmargin=*,nosep]
...
pretrained Wan weights. Its latent velocity MSE gives alpha-derived foreground latent cells weight 2 and
background cells weight 1, again normalised by the mean weight. An auxiliary term of weight 1 decodes both
the predicted clean latent and the target latent through a separately trained frozen video codec. The term,
gated by $(1-t)$, combines alpha disagreement, squared RGB disagreement where target and prediction are
foreground, and a 0.1-weighted background RGB penalty. The codec uses fourfold temporal and spatial
compression with eight latent channels (f4t4d8). This is a compact Wan-architecture reference, not a
Wan2.1 reproduction. We release checkpoints from
...
64-frame scores are different measurements. Figure~\ref{fig:dit-samples} shows the warm-started model.
...
core Colab therefore follows the pixel-space route; it is a choice for this dataset and this codec.
